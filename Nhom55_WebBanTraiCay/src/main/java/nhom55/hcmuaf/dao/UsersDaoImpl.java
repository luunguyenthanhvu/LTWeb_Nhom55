...
public class UsersDaoImpl implements UsersDao {
  ...
  public Users updateProfileNoImage(int userId, String newUserName, String newEmail,
    ...
    return JDBIConnector.get().withHandle(handle -> {
      int rowCount = handle.createUpdate(
              "UPDATE Users SET username = :username, email = :email, address = :address, phoneNumber = :phoneNumber, dateOfBirth = :dateOfBirth, sexual =:sexual WHERE id = :id")
          .bind("id", userId)
      ...
      return (rowCount > 0) ? handle.createQuery(
              "SELECT id, username, email, address, phoneNumber, dateOfBirth, img, sexual FROM Users WHERE id = :id")
          .bind("id", userId)
          .mapToBean(Users.class)
          .findOne()
          .orElse(null) : null;
  ...
  public Users updateProfileWithImage(int userId, String newUserName, String newEmail,
    ...
    return JDBIConnector.get().withHandle(handle -> {
      ...
      return (rowCount > 0) ? handle.createQuery(
              "SELECT username, email, address, phoneNumber, dateOfBirth, img, sexual FROM Users WHERE id = :id")
          .bind("id", userId)
          .mapToBean(Users.class)
          .findOne()
          .orElse(null) : null;
