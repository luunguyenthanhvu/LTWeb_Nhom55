...
public class BillDaoImpl implements BillDao {
    ...
    public int getIDAListProductFromBills(LocalDateTime orderedDate, int idUser) {
        ...
        result = JDBIConnector.get().withHandle(handle -> {
           return handle.createQuery("SELECT id from bills where orderedDate = :orderedDate and userId = :idUser ")
                    .bind("orderedDate",orderedDate)
                    .bind("idUser",idUser)
                    .mapTo(int.class).one();
        });
    ...
    public boolean addAProductToBillDetails(int idProduct, int idBills,int quantity, double totalPrice) {
        ...
        count = JDBIConnector.get().withHandle(h ->{
            return h.createQuery("SELECT count(*) from bill_details where idProduct = :idProduct and idBills= :idBills")
         ...
         if(count >0) {
             return false;
              ...
              JDBIConnector.get().withHandle( h -> {
                  return   h.createUpdate("INSERT INTO bill_details(quantity,totalPrice,idProduct,idBills) values (:quantity,:totalPrice,:idProduct,:idBills)")
               .bind("idProduct",idProduct)
                         .bind("idBills",idBills)
    ...
    public int countResultSearchingBill(String txtSearch) {
        return JDBIConnector.get().withHandle(h ->
                h.select("SELECT count(*) FROM bills WHERE firstName LIKE ? OR lastName LIKE ?", "%" + txtSearch + "%", "%" + txtSearch + "%")
                        .mapTo(Integer.class)
                        .one()
        );
    }
