        ...
        List<Users> result = JDBIConnector.get().withHandle(handle -> {
            ...
            try {
                ...
                List<Users> resultList = handle.createQuery(
                                "with testThu as (select ROW_NUMBER() over (order by id asc) as r, id, username, hash, email, phoneNumber, address, status, img, dateOfBirth, sexual, role from users where userName LIKE ?)\n" +
